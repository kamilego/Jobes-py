...
def first_steps(base_path, new_project, dwg_dir, dic_w_replace):
    while True:
        ...
        for i in range(len(l)):
            for key, value in dic_w_replace.items():
                l[i] = replace_second(l[i], key, value)
...
def edit_excel_histry(list_values):
    while True:
        ...
        for num in range(length):
            first_set.add(ws["A"+str(num+5)].value)
        if len(first_set - second_set) == 0:
            print("List of folders in excel and path are the same\nNothing has been added.")
            break
        row_num = list_values.index(list(second_set - first_set)[0])+5
        ws.insert_rows(row_num)
        second_set = list(second_set)
        second_set.sort()
        for num, elem in enumerate(second_set, start=5):
